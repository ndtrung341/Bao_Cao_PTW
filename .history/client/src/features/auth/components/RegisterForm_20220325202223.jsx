...
const schema = yup.object().shape({
   username: yup
      ...
      .test({ test: (value) => value.split(/\s+/).length >= 2, message: 'Họ tên phải có 2 từ trở lên' }),
...
const RegisterForm = ({ onRegister }) => {
   const { control, handleSubmit } = useForm({ defaultValues, mode: 'all', resolver: yupResolver(schema) });
